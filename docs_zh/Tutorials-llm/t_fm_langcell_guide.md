...
双塔架构（文本 + 细胞），将自然语言描述与细胞嵌入进行对齐

!!! tip "何时选择 LangCell"

    适用于需要文本引导的细胞检索、自然语言细胞查询或文本-细胞对齐的场景
...
|-------------|--------|
| **基因 ID 方案** | symbol |
| **预处理** | 标准预处理。文本查询可以是细胞类型或状态的自然语言描述。 |
| **数据格式** | AnnData (`.h5ad`) |
...
| 键 | 位置 | 描述 |
|-----|----------|-------------|
| `X_langcell` | `adata.obsm` | 细胞嵌入向量（512 维） |
...
- **仓库 / 检查点：** [https://github.com/langcell/LangCell](https://github.com/langcell/LangCell)
- **许可证：** 请查阅上游 LICENSE
...
如需包含代码的逐步演练，请参阅 [LangCell 教程笔记本](t_fm_langcell.ipynb)。
