...
用于单细胞分析的对话式聊天界面，通过对话进行零样本注释

!!! tip "何时选择 ChatCell"

    适用于希望通过交互式对话进行细胞分析、会话式注释或对话驱动探索的场景
...
| 键 | 位置 | 描述 |
|-----|----------|-------------|
| `X_chatcell` | `adata.obsm` | 细胞嵌入向量（512 维） |
| `chatcell_pred` | `adata.obs` | 预测的细胞类型标签 |
...
- **仓库 / 检查点：** [https://github.com/chatcell/CHATCELL](https://github.com/chatcell/CHATCELL)
- **许可证：** 请查阅上游 LICENSE
...
如需包含代码的逐步演练，请参阅 [ChatCell 教程笔记本](t_fm_chatcell.ipynb)。
